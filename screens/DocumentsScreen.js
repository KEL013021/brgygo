...
export default function DocumentsScreen({ navigation }) {
  ...
  const fetchRequests = async (userId) => {
    try {
      const res = await fetch('http://192.168.0.93/RESIDENT_COPY1/database/get_request.php', {
        method: 'POST',
  ...
  useEffect(() => {
    fetch('http://192.168.0.93/RESIDENT_COPY1/database/get_services.php')
      .then(res => res.json())
  ...
  const handleSubmit = () => {
    ...
    fetch('http://192.168.0.93/RESIDENT_COPY1/database/submit_request.php', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
